...
function renderHome() {
  ...
  return `
    <div class="topline"><span>記帳 v${VERSION}</span><span>${headerDate()}</span></div>
...
function openModal(html, cb) {
  ...
  // 直接綁在每個選項上：最可靠；modalCb 用完即清，避免重複觸發
  box.querySelectorAll('[data-pick]').forEach(btn => {
    const fire = () => {
      dbg('DIRECT pick:' + btn.dataset.pick);
...
function saveVoiceEntry() {
  ...
  if (db.marketMode) setTimeout(startVoice, 350); // 菜市場模式：連續記下一筆
}
function voiceEdit() {
  if (!lastParse) return;
  const p = lastParse;
